...
export default function stripJsonComments(
	...
	let isInsideSqrtBrackets = false
	...
	for (let index = 0; index < jsonString.length; index++) {
		...
		if (
			...
			jsonString =
				...
				sqrtResult +
				jsonString.slice(index + 1)
			index += sqrtResult.length
		}
		...
		if (!isInsideComment && currentCharacter + nextCharacter === "//") {
			...
			sqrtResult = "" // reset the sqrt Value
			...
			isInsideSqrtBrackets = true
		} else if (isInsideComment === multiComment && nextCharacter === "]") {
			index += 1
			isInsideSqrtBrackets = false
		} else if (
			isInsideComment === multiComment &&
			currentCharacter + nextCharacter === "*/"
		) {
